...
def test_get_pairing_json_obj():
    ...
    assert list(valid_dict.keys()) == ["title", "authors", "genres", "cover_link", "pairing", "instructions", "information"]
    assert type(valid_dict["instructions"]) is list 
    assert type(valid_dict["information"]) is str 
    assert type(valid_dict["pairing"]) is str 
    ...
    assert list(invalid_dict.keys()) == ["title", "authors", "genres", "cover_link", "pairing", "instructions", "information"]
    assert invalid_dict["title"] == ""
    assert invalid_dict["authors"] == []
    assert invalid_dict["genres"] == []
    assert invalid_dict["cover_link"] == ""
    assert invalid_dict["pairing"] == book_invalid.get_no_match_drink()["pairing"]
...
def test_get_pairing():
    names_of_drinks = [drink["name"] for drink in book_valid.get_matching_drinks()]
    assert book_valid.get_pairing()["pairing"] in names_of_drinks
